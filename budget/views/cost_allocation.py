''' View that generates a cost allocation report for AWS '''
...
@view_config(route_name='cost_allocation',
             match_param='type=by_account',
             renderer='budget:templates/cost_allocation.pt')
def cost_allocation_by_account(request):
    ...
    for line in cost_lines:
        data = {'name' : 'data',
                'content' : {'usage_type' : line.usage_type,
...
#
    ...
    rendered = []
    for obj in root:
        rendered.extend(obj.dump())

    return {'data' : rendered,
            'selectors' : {'date' : {'selected' : selected_date,
                                     'list' : dates
                                    }
                          }
           }
...
def cost_allocation_graph(request):
    ...
    if request.params.get('selected_date', None):
        selected_date = datetime.strptime(request.params['selected_date'],
                                          "%Y-%m-%d")
    ...
    results = DBSession.query(AwsCostAllocation.total_cost,
    ...
    for result in results:
        ...
        graph_data.append({'label': result.billing_period_start_date.strftime("%Y-%m-%d"),
                           'value': "%.3f" % Decimal(result.total_cost)})

    # setting y_domain because the default calculation often misses the max value
    graph = DiscreteBarChart(yDomain=[0, max([r.total_cost for r in results])],
                             showXAxis=False)
