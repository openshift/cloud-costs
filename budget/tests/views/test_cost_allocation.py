...
class TestCostAllocationGraph(unittest.TestCase):
    def setUp(self):
        ...
        Base.metadata.create_all(engine)
        with transaction.manager:
            cost = AwsCostAllocation(invoice_id='invoice id',
                                     payer_account_id=0,
                                     linked_account_id=0,
...
class TestCostAllocationByAccount(unittest.TestCase):
    ...
    def runTest(self):
        ...
        self.assertEqual(info['selectors']['date']['selected'], yesterday)
